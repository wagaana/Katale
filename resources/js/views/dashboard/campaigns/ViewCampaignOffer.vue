              ...
              <div class="mb-3 row">
                <div class="col-md-6 col">
                  <div class="col-md-12">
                    ...
                    <div>
                      <span
                        v-for="(parameter, i) in campaignOffer.parameters"
                        :key="i"
                      >
                        <v-file-input
...
export default {
  ...
  computed: {
    getEmailTemplateHtml() {
      ...
      for (const index in context.campaignOffer.parameters) {
        let keyToReplace = context.campaignOffer.parameters[index].value;
        let placeHolder = context.campaignOffer.parameters[index].placeHolder;
        const inputType = context.campaignOffer.parameters[index].inputType;
        if (inputType === "file") {
  ...
  watch: {
    ...
    getUploadedCampaignOfferFile(val) {
      val &&
        setTimeout(() => {
          let index = this.campaignOffer.parameters.findIndex(
            (el) => el.id == parseInt(val.reference)
          );
          this.campaignOffer.parameters[index].placeHolder = val.fileName;
          this.overlay = false;
        }, 3000);
    },
  },
  ...
  methods: {
    ...
    appendSelectedParam(field) {
      if (this.activeField == null) {
        ...
        let placeHolder = this.campaignOffer.parameters[index].placeHolder;
