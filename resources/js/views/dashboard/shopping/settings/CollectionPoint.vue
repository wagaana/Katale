    ...
    <v-row>
      <v-col cols="12" class="py-0">
        <h2 class="font-weight-light mb-0">
          {{ getCollectionPointDetails.label }}
        </h2>
        <span class="overline">{{ getCollectionPointDetails.address }}</span>
      </v-col>
          ...
          <v-row class="px-4">
            <v-col cols="12" sm="4" order-sm="2">
              <v-card min-width="150" elevation="12">
                <v-img :src="`${getCollectionPointDetails.imageUrl}`" />
              </v-card>
            </v-col>
            ...
            <v-col cols="12" sm="8" order-sm="2">
              <v-card
                ...
                class="py-2"
              >
                <CollectionPointConfiguration
                  :data="getCollectionPointDetails"
                />
                ...
                <v-row class="px-4">
                  ...
                  <v-col cols="12" class="py-2">
                    <div v-html="getCollectionPointDetails.description"></div>
                  </v-col>
                </v-row>
              </v-card>
            </v-col>
      ...
      <v-col cols="12" sm="12" order-sm="2">
        <BranchPhotosAndAdmins :data="getCollectionPointDetails" />
      </v-col>
    </v-row>
  </div>
</template>
...
import CollectionPointConfiguration from "../../../../components/CollectionPointConfiguration.vue";

export default {
  template: "#CollectionPointDetails",
  ...
  created() {
    this.$store.dispatch("fetchCollectionPointDetails", this.$route.params.id);
    this.$store.dispatch("fetchCollectionPointAdmins", this.$route.params.id);
    this.$store.dispatch("fetchPlasticCategories");
  },
  ...
  methods: {
    handleImageChange(event) {
      ...
      formData.append("collectionPointId", this.$route.params.id);
      this.$store.dispatch("sendCollectionPointGalleryImage", formData);
    },
    ...
    validate() {
      if (this.$refs.pricingForm.validate()) {
        // send it
        this.newPlasticPricing.collectionPointId = this.$route.params.id;
